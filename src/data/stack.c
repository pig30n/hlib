...
Int8 peak_stack(const struct stack *s, Void** data)
{
     if(!stack) return 1;
...
Int8 pop_stack(struct stack *s, Void **data)
{
     if(!stack) return 1;
     if(stack->cnt>=s->max) return 2;
     if(!data) return 3;
...
Int8 push_stack(struct stack *stack, const Void *data)
{
     ...
     stack->data[(stack->cnt)++] = data;
     return 0;
}

Int8 print_stack(const File *stream, struct stack *stack, const Char *format)
{
     if(!stack) return = 1;
